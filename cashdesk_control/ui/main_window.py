...
if QT_AVAILABLE:
    ...
    class MainWindow(QMainWindow):
        ...
        def __init__(self, parent: QWidget | None = None) -> None:
            ...
            self._profile_settings = SettingsStore(self._paths.profiles_file)
            try:
                self._profile_settings.load()
            except Exception as exc:
                self._logger.warning("profiles could not be loaded: %s", exc)
            self._profile_store = ProfileStore(self._profile_settings)
        ...
        def _restore_saved_profiles(self) -> None:
            ...
            for profile in profiles:
                if profile.profile_id in {snapshot.session_id for snapshot in self.session_manager.snapshots()}:
                    continue
                try:
                    self.session_manager.create(profile)
        ...
        def _open_new_connection(self) -> None:
            ...
            try:
                ...
                try:
                    ...
                    self._logger.warning("profile could not be persisted: %s", exc)
                    self.show_status("Подключение создано, но профиль не удалось сохранить")
